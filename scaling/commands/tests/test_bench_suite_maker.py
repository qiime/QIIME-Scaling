...
class BenchSuiteMakerTests(TestCase):
    def setUp(self):
        ...
        self.command3 = ("parallel_pick_otus_uclust_ref.py -r ref_file.fna -i "
                         "input.fna")
        ...
        self.bench_files_single = [["1000000.fna"],
                                   ["2000000.fna"],
                                   ["3000000.fna"]]
        self.bench_files_mult = [["reads/1000000.fna", "barcodes/1000000.fna"],
        ...
        self.pbs = True
        self.job_prefix = "test"
        self.queue = "friendlyq"
        self.pbs_extra_args = "-m abe"
    ...
    def test_pbs_files_suite(self):
        """Bench suite correctly generated in a PBS environment"""
        obs = self.cmd(command=self.command,
                       bench_files=self.bench_files_single,
                       in_opts=self.in_opts_single,
                       pbs=self.pbs,
                       ...
                       queue=self.queue,
                       pbs_extra_args=self.pbs_extra_args)
        self.assertEqual(obs.keys(), ['bench_suite'])
        obs = obs['bench_suite']
        self.assertEqual(obs, pbs_file_suite)
    ...
    def test_pbs_parameter_suite(self):
        """"""
        obs = self.cmd(command=self.command3,
                       parameters=self.param_mult,
                       pbs=self.pbs,
        ...
        self.assertEqual(obs.keys(), ['bench_suite'])
        obs = obs['bench_suite']
        self.assertEqual(obs, pbs_parameter_suite)
